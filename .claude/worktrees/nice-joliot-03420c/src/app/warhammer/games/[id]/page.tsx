...
function buildMarkers(army: ArmyRow, player: "P1" | "P2"): UnitMarker[] {
  ...
  entries.forEach((entry) => {
    ...
    for (let i = 0; i < Math.max(1, entry.quantity); i++) {
      ...
      const startingModels = Math.max(1, entry.modelCount > 0 ? entry.modelCount : unit.models.min);
      markers.push({
        id: uid,
        unitId: entry.unitId,
        unitName: unit.name,
        ...
        modelCount: startingModels,
        woundsPerModel: unit.stats.wounds,
        startingModelCount: startingModels,
        stats: unit.stats,
...
export default function WarhammerGameRoom() {
  ...
  const [overwatchPending, setOverwatchPending] = useState<{ attackerId: string; targetId: string } | null>(null);
  ...
  function undoAction() {
    setHistory((prev) => {
      ...
      const current = { markers, p1Cp, p2Cp, p1Vp, p2Vp };
  ...
  function resolveBattleShock(player: "P1" | "P2", currentMarkers: UnitMarker[]): UnitMarker[] {
    ...
    for (const m of friendlyActive) {
      const isMultiModel = (m.startingModelCount ?? 1) > 1;
      const belowHalf = isMultiModel
        ? (m.modelCount ?? 1) < Math.ceil((m.startingModelCount ?? 1) / 2)
      ...
      const strengthNote = isMultiModel
        ? `${m.modelCount ?? 1}/${m.startingModelCount ?? 1} models`
        : `${m.currentWounds}/${m.maxWounds}W`;
      addLog(
        `Battle-shock: ${m.unitName} below half-strength (${strengthNote}) — 2D6 (${r1}+${r2}=${total}) vs Ld${ldNum}+ → ${shocked ? "SHOCKED!" : "holds."}`,
        player
      );
  ...
  // After first player's fight: score → second player's command phase
  // After second player's fight: score → end-of-round scoring phase
  function advancePhase() {
    ...
    if (gamePhase === "fight") {
      ...
      if (activePlayer === firstPlayer) {
        ...
        addLog(`${firstPlayer}'s turn complete. Round ${round} — ${secondPlayer}'s Turn: Command Phase.`, "system");
        giveCommandPhaseCP(secondPlayer, markers);
        if (secondPlayer === "P2" ? p2Hand.length < 3 : p1Hand.length < 3) drawMission(secondPlayer);
      } else {
        setGamePhase("scoring");
        setDestroyedThisTurn([]);
        setDestroyedThisPhase([]);
        setAdvancedThisTurn([]);
        addLog(`${secondPlayer}'s turn complete. End of Round ${round} — click "End Round" to continue.`, "system");
      }
      return;
    }
    ...
    if (idx >= 0 && idx < PLAYER_PHASES.length - 1) {
      ...
      // Reset fight-back when entering fight phase
      if (next === "fight") setFightBackMode(false);
      setGamePhase(next);
  ...
  function endRound() {
    ...
    setCombat(INIT_COMBAT);
    setMoveUnit(null);
    if (gameMode === "solo") setSoloSide(firstPlayer);
    addLog(`Round ${nextRound} begins. ${firstPlayer}'s Turn — Command Phase.`, "system");
    giveCommandPhaseCP(firstPlayer, markers);
    if (firstPlayer === "P1" ? p1Hand.length < 3 : p2Hand.length < 3) drawMission(firstPlayer);
  }
  ...
  function handleCellClick(x: number, y: number) {
    ...
    if (gamePhase === "movement" && moveUnit) {
      ...
      if (!marker) return;
      if (movedThisTurn.has(moveUnit)) {
        addLog(`${marker.unitName} has already moved this turn.`, "system");
        return;
      }
      ...
      // Ruins are passable (area terrain): units CAN end inside ruins
      pushHistory();
      const oldX = marker.x;
      const oldY = marker.y;
      setMarkers((prev) =>
      ...
      setMovedThisTurn((prev) => new Set([...prev, moveUnit]));
      if (moveAdvance) {
  ...
  function handleUnitClick(markerId: string) {
    ...
    if (roomPhase === "game") {
      ...
      if (gamePhase === "fight") {
        const activeSide = gameMode === "solo" ? soloSide : activePlayer;
        const inactiveSide: "P1" | "P2" = activeSide === "P1" ? "P2" : "P1";
        const fightingSide = fightBackMode ? inactiveSide : activeSide;

        if (combat.step === "idle" && m.player === fightingSide && !foughtThisPhase.has(markerId)) {
          const hasMelee = m.weapons.some((w) => w.type === "Melee");
          if (!hasMelee) { addLog(`${m.unitName} has no melee weapons.`, "system"); return; }
          if (fightBackMode) {
            const inEngagement = markers.some(
              (e) => e.player !== m.player && !e.isDestroyed && !e.isInReserve &&
                Math.sqrt((e.x - m.x) ** 2 + (e.y - m.y) ** 2) <= 2
            );
            if (!inEngagement) { addLog(`${m.unitName} is not in engagement range.`, "system"); return; }
          }
  ...
  // ── Shooting / Fight resolution ──
  function resolveHitRolls() {
    const attacker = markers.find((m) => m.id === combat.attackerId);
    if (!attacker || combat.weaponIdx === null) return;
    const weapon = attacker.weapons[combat.weaponIdx];
    const modelCount = attacker.modelCount ?? 1;
    let numAttacks = 0;
    for (let i = 0; i < modelCount; i++) numAttacks += parseDiceExpr(weapon.attacks);
    const skillTarget = parseSkill(weapon.skill);
    const rolls = rollDice(numAttacks);
    const hits = rolls.filter((r) => r >= skillTarget).length;
    const actionType = gamePhase === "fight" ? "Fight" : "Shooting";
    const modelNote = modelCount > 1 ? ` (${modelCount} models)` : "";
    addLog(`${actionType}${modelNote}: ${numAttacks} attack(s) → ${rolls.join(", ")} → ${hits} hit(s) (needing ${skillTarget}+).`, attacker.player);
  ...
  function resolveSaveRolls() {
    ...
    const totalDmgLog = unsaved * dmgPer;
    addLog(`Saves: ${coverSave}+ needed (Sv${saveBase}, AP${ap}${targetInRuins ? ", +1 cover" : ""}) → ${rolls.join(", ")} → ${unsaved} unsaved → ${totalDmgLog} damage.`, target.player);
    showRoll({ rolls, type: "save", threshold: coverSave, label: `Save Rolls (${coverSave}+)` });

    // Apply damage model-by-model (damage does not spill across models per 10th ed rules)
    const currentTarget = markers.find((m) => m.id === combat.targetId)!;
    const woundsPerModel = currentTarget.woundsPerModel ?? currentTarget.maxWounds;
    let modelCount = currentTarget.modelCount ?? 1;
    let currentWounds = currentTarget.currentWounds;
    let modelsKilled = 0;

    for (let i = 0; i < unsaved && modelCount > 0; i++) {
      currentWounds -= dmgPer;
      if (currentWounds <= 0) {
        modelCount--;
        modelsKilled++;
        currentWounds = modelCount > 0 ? woundsPerModel : 0;
      }
    }

    const targetDestroyed = modelCount <= 0;
      ...
      prev.map((m) => {
        if (m.id === targetId) {
          if (targetDestroyed) {
            ...
            addLog(`${m.unitName}: ${modelsKilled} model(s) slain (${modelCount}/${m.startingModelCount ?? 1} remaining).`, "system");
          }
          return { ...m, currentWounds, modelCount: Math.max(0, modelCount), isDestroyed: targetDestroyed };
        }
        if (m.id === attackerId) {
          return isFightPhase
            ? { ...m, hasFought: true }
  ...
  function resolveCharge(attackerId: string, targetId: string) {
    ...
    if (total >= Math.ceil(dist)) {
      ...
      // Offer defender the option to Overwatch (1 CP stratagem)
      setOverwatchPending({ attackerId, targetId });
    } else {
  ...
  function resolveOverwatchYes() {
    if (!overwatchPending) return;
    const { targetId } = overwatchPending;
    const defender = markers.find((m) => m.id === targetId);
    if (!defender) { setOverwatchPending(null); return; }
    const defCP = defender.player === "P1" ? p1Cp : p2Cp;
    if (defCP < 1) { addLog("Not enough CP for Overwatch!", "system"); setOverwatchPending(null); return; }
    if (defender.player === "P1") setP1Cp((n) => n - 1);
    else setP2Cp((n) => n - 1);
    const owRolls = rollDice(1);
    const owHits = owRolls.filter((r) => r === 6).length;
    if (owHits > 0) {
      addLog(`Overwatch: ${defender.unitName} — ${owRolls.join(",")} — ${owHits} hit(s)!`, defender.player);
    } else {
      addLog(`Overwatch: ${owRolls.join(",")} — no hits.`, defender.player);
    }
    setOverwatchPending(null);
  }

  function resolveOverwatchNo() {
    if (!overwatchPending) return;
    const defender = markers.find((m) => m.id === overwatchPending.targetId);
    if (defender) addLog(`${defender.unitName} declines Overwatch.`, defender.player);
    setOverwatchPending(null);
  }
        ...
        <div className="space-y-2">
          ...
          {combat.step === "selectTarget" && (
            <div>
              ...
              <button
                onClick={() => setCombat((prev) => ({ ...prev, step: "selectWeapon", weaponIdx: null }))}
                className="flex items-center gap-1 text-[10px] py-1 px-2 rounded"
                style={{ color: "var(--text-muted)", backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}
              >
                <ArrowLeft size={9} /> Back to weapon
              </button>
            </div>
          )}
