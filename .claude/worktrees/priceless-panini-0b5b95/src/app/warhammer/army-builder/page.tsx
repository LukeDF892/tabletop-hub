...
// TODO: Set maxDetachments > 1 to allow house rule multi-detachment armies
const MAX_DETACHMENTS = 1;
...
export default function ArmyBuilderPage() {
  ...
  const saveArmy = useCallback(async () => {
    if (!faction || !detachment || !gameSize || army.length === 0) return;
    if (!armyName.trim()) {
      setSaveError("Enter an army name before saving.");
      return;
    }
    setSaving(true);
    setSaveError(null);
    try {
      ...
      if (!user) { setSaveError("Sign in to save armies."); return; }

      const pts = army.reduce((sum, e) => sum + e.unit.points * e.quantity, 0);
      const payload = {
        user_id: user.id,
        name: armyName.trim(),
        faction: faction.name,
        detachment: detachment.name,
        detachments: [detachment.name],
        total_points: pts,
        points_limit: gameSize.points,
        units: army.map(e => ({ id: e.unit.id, name: e.unit.name, quantity: e.quantity, modelCount: e.modelCount, points: e.unit.points })),
      };

      if (editArmyId) {
        const { error } = await supabase.from("warhammer_armies").update(payload).eq("id", editArmyId).eq("user_id", user.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("warhammer_armies").insert(payload);
        if (error) throw error;
      }
  ...
  }, [faction, detachment, gameSize, army, armyName, editArmyId, router]);
            ...
            >
              ...
              <div className="px-5 pb-5 flex-shrink-0 space-y-2">
                ...
                <button
                  ...
                  onClick={saveArmy}
                >
                  {saving ? "Saving…" : `${editArmyId ? "Update" : "Save"} Army · ${totalPts}pts`}
                </button>
              </div>
            </div>
          </main>
